...
with open('orders_export.csv', mode='rt') as f, open('sorted.csv', 'w') as final:
...
for row1 in itertools.izip_longest(csv_f1):
    if countLine == 0:
        ...
        if orderNumber == row1[0][1] or len(entrega)< 6:
            ...
            try:
                ...
                try:
                    ...
                    if(row1[0][17].find('Medio kilo') != -1):
                        otherLine[index-1][products[splited[0]]]="1/2 KG"
                    else:
            ...
            countLine2 =0
            countLine3 =0
            for row3 in itertools.izip_longest(csv_f3):
...
with open('entrega_usuarios.csv', 'wb') as writeFile:
    writer = csv.writer(writeFile,delimiter=',')
